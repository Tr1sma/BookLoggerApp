...
## Status (Stand 2026-06-19) — ✅ abgeschlossen

**Alle 10 Phasen umgesetzt.** Sämtliche `hoch`+`mittel` in-scope-Befunde stehen in `CODE_REVIEW_TODO.md` auf `- [x]` — einzige Ausnahme **LOG-05**, das re-verifiziert als Nicht-Defekt **verworfen** wurde (Widget==App, identische Integer-Division). Zusätzlich wurden mehrere `niedrig`-Befunde miterledigt und in der TODO abgehakt (u.a. BookRepository-`ct`-Durchreichung, Streak-/BookDetail-Lokalzeit, ReadingGoal.IsActive-Prädikat, AppSettingsProvider-Kommentare, PaywallVM-Reentrancy, Restore-Rollback-Härtung, Wishlist/Tropes-Service-Gating).

- **Unit-getestet & grün** (`dotnet test`): Phasen 1–3, 6–10 sowie die Core-Logik von 4–5.
- **Nur manuell/On-Device verifizierbar** (Tests-Projekt = `net10.0`, kann das MAUI-Projekt nicht referenzieren): Android-Billing/PaywallModal (Phase 4), Manifest/MainActivity/WebView/AppRestart (Phase 5), Widget-Render-Anteile (Phase 7 & 10).
- **Verbleibend (bewusst offen, `niedrig`/out-of-scope):** zwei nur teilweise behobene `niedrig`-Befunde sind in der TODO mit `TEILWEISE BEHOBEN` + Offen-Punkt markiert (async-void `Stats.razor` `OnLocationChanged`; `ProgressService.EndSessionAsync` nicht atomar über die Coin-/XP-Kette). Die restlichen `niedrig`-Items waren nie Teil dieses Plans.
...
- **Abschluss:** ✅ LOG-05 in der TODO mit Notiz „VERWORFEN (re-verifiziert V12): kein Defekt (Widget==App)" markiert.

## Scope-Zusammenfassung
- **64 hoch+mittel** im Scope; **63 aktioniert (alle behoben & abgehakt)**, **1 verworfen (LOG-05, re-verifiziert als Nicht-Defekt)**. → **abgeschlossen.**
- 23 der 24 `⚠`-Befunde re-verifiziert bestätigt; 0 widerlegt.
- 72 `niedrig`-Befunde sind nicht Teil dieses Plans — davon wurden einige **miterledigt** (in der TODO als `BEHOBEN (V12)` abgehakt), zwei sind **teilweise** behoben (mit Offen-Punkt markiert).
