...
public class GoalService : IGoalService
{
    ...
    public GoalService(IUnitOfWork unitOfWork, IAnalyticsService? analytics = null, IFeatureGuard? featureGuard = null, IValidationService? validationService = null)
    {
    ...
    {
        // CODE_REVIEW BUG-05: enforce the goal rules (title, target range, end-after-start,
        // max one-year period) before persistence.
        if (_validationService is not null)
            await _validationService.ValidateAndThrowAsync(goal, ct);
