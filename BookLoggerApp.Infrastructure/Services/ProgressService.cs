...
public class ProgressService : IProgressService
{
    ...
    {
        // CODE_REVIEW BUG-05: validate the manually-logged session (book, started-at, minutes,
        // pages, xp ranges) before any XP/streak/coin side-effects run or anything is persisted.
        if (_validationService is not null)
            await _validationService.ValidateAndThrowAsync(session, ct);
