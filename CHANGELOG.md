...
### Behoben
- Abgelaufene Abonnements werden jetzt vollständig herabgestuft: Ablauf-Prüfung beim App-Start und beim Hintergrund-Aktualisieren speichert den neuen Free-Status jetzt in der Datenbank und läuft den Entitlement-Lapse-Handler durch (versteckt überschüssige Regale, Pflanzen, Dekorationen). Bisher wurde der Tier im Speicher auf Free gesetzt, die Datenbank blieb jedoch bei Plus/Premium und die Datenschutz-Regeln wurden nie angewendet.
- Aktive Auto-Renewing-Abonnements werden beim App-Start nicht mehr fälschlich herabgestuft: Wenn der Billing-Provider signalisiert, dass ein Abo aktiv verlängert wird, wird der geschätzte Ablaufzeitpunkt (+30/+365 Tage ab Kaufdatum) jetzt ignoriert — ältere Abonnements, deren geschätzter Ablauf in der Vergangenheit liegt, blieben dadurch beim Neustart im Plus/Premium-Tier.
- Promo-Gutschriften (kostenloser zeitlich begrenzter Testzugang) werden beim App-Resume jetzt nicht mehr fälschlich auf Free herabgestuft, wenn kein aktives Play-Abo gefunden wird. Promos nutzen kein Play-Billing-Token; die Resume-Prüfung übersieht sie daher und hat sie irrtümlich als abgelaufenes Abo behandelt.
- Kauf-Bestätigung (Acknowledge) an Google Play sendet jetzt den korrekten PurchaseToken statt einem internen Bezeichner des Plugins, der von Play nicht akzeptiert wird — fehlerhafte Bestätigungen führen zum automatischen Rückerstattung nach 3 Tagen.
- Bei unbekannter ProductId nach einem Kauf (z. B. Play-Katalog und App nicht synchron) wird jetzt ein Fehler zurückgegeben statt fälschlich „Kauf erfolgreich" zu melden, während der Tier unverändert auf Free bleibt.
- Beim Wechsel von Plus auf Premium wird das vorhandene Abo-Token jetzt für das Proration-Update an Google Play übergeben, statt immer einen Neu-Kauf zu starten.
- Intro-Angebots-Badge im Paywall zeigt jetzt das korrekte Angebot je SKU: Bisher erschien das Badge immer beim monatlichen Tarif beider Stufen, unabhängig davon ob für diese konkrete SKU tatsächlich ein Einführungsangebot existiert. Das Badge wird jetzt pro Stufe einzeln gesetzt.
...
- Eingabe-Validierung greift jetzt tatsächlich: Die FluentValidation-Regeln für Bücher, Lesesitzungen, Leseziele und Pflanzen waren registriert, wurden aber auf keinem produktiven Speicherpfad aufgerufen — ungültige Daten (leere Titel/Autoren, unsinnige Seitenzahlen, Sitzungen > 24 h usw.) gelangten ungeprüft in die Datenbank. Buch- und Ziel-Anlage, das Buch-Speichern (inkl. Genres/Regale/Tropes), das Hinzufügen von Lesesitzungen und der Pflanzenkauf prüfen die Daten nun, bevor etwas gespeichert wird; beim Pflanzenkauf läuft die Prüfung vor dem Abbuchen der Münzen. Das reine Bearbeiten einzelner Felder (z. B. Bewertung/Notiz) eines bereits gespeicherten Buchs bleibt bewusst ungeprüft, damit Altbestände mit historischen Inkonsistenzen weiterhin bearbeitbar sind.
- Bildschirm-Ladevorgänge sind jetzt durchgängig abbrechbar (CancellationToken): Die spezifischen Repository-Methoden, die UnitOfWork-basierten Services (Bücher, Ziele, Fortschritt, Statistiken, Pflanzen, Prognose) und die datenladenden ViewModels reichen den Abbruch-Token jetzt bis zur Datenbankabfrage durch. Beim erneuten Laden eines Bildschirms wird ein noch laufender Ladevorgang abgebrochen, statt parallel auf demselben Datenbank-Kontext weiterzulaufen; ein so abgebrochener Ladevorgang erzeugt keine irreführende Fehlermeldung mehr und kann den Zustand eines bereits gestarteten neuen Ladevorgangs nicht mehr überschreiben.

### Geändert
- Leseziele werden jetzt beim Anlegen und Bearbeiten gegen die dokumentierten Regeln geprüft (Titel erforderlich, Ziel zwischen 1 und 100 000, Enddatum nach Startdatum, Laufzeit höchstens ein Jahr). Ziele, die diese Grenzen überschreiten, werden nun abgelehnt — zuvor wurden diese Regeln nie erzwungen.
